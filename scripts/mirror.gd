...
@export var level_scene = "res://scenes/mirror_level.tscn"  # Altere para o caminho da cena do espelho
...
func interact():
	if not GameState.mirror_completed:  # Mudamos o baú para espelho
		GameState.show_dialog_sequence(
			[
				{"name": "", "text": "[font_size=20](Aproximando-se, Noé vê não o seu reflexo normal, mas diluído entre várias versões de si e da mãe.)", "color": GameState.cores["noé"]},
			],
			false,
			"",
			"",
...
func show_nothing_happens_dialog():
	if GameState.mirror_good_choice:  # Alterei a lógica para espelho
		GameState.show_dialog_sequence(
			[
				{"name": "Noé", "text": "Agora posso tentar compreender-me a mim mesmo.", "color": GameState.cores["noé"]},
			],
			false, "", "", self, "_on_nothing_dialog_finished")
	else:
		GameState.show_dialog_sequence(
			[
				{"name": "Noé", "text": "Este reflexo não é meu...", "color": GameState.cores["noé"]},
			],
			false, "", "", self, "_on_nothing_dialog_finished")
...
func update_appearance():
	...
	if not GameState.mirror_completed:
		sprite.texture = load("res://assets/ViagemDeNoe/mirror_neutral.png")  # Troquei a textura para o espelho
	elif GameState.mirror_good_choice:
		sprite.texture = load("res://assets/ViagemDeNoe/mirror_good.png")  # Troquei a textura para o espelho
	else:
		sprite.texture = load("res://assets/ViagemDeNoe/mirror_bad.png")  # Troquei a textura para o espelho
